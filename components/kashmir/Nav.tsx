 ...
 *
 * The mobile drawer is rendered as a SIBLING of the (transformed) header, not
 * a child: a `translate` on the header would otherwise become the containing
 * block for the drawer's `position: fixed`, collapsing its height and leaving
...
export default function Nav() {
  ...
  return (
    <>
      ...
      {/* mobile drawer — sibling of the header, full-screen, opaque */}
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            key="mobile-drawer"
            ...
            className="k-root fixed inset-0 z-[60] bg-[var(--paper)] lg:hidden"
          >
            <div className="flex h-[70px] items-center justify-between border-b k-hair px-6">
              <span className="k-serif text-[1.3rem] tracking-[0.3em] text-[var(--ink)]">{brand.name}</span>
              <button
                type="button"
            ...
            <div className="flex h-[calc(100%-70px)] flex-col justify-between px-6 py-10">
